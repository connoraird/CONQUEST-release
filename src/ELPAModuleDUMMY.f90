...
contains
   subroutine init_ELPA (matrix_size, row_size, col_size, desc, info)
      ...
      if(use_elpa) then
       call cq_abort("initi_ELPA: init_ELPA is called though ELPA_module is not compiled")
      else
       call cq_abort("initi_ELPA: init_ELPA is called even though use_elpa is false")
      endif
